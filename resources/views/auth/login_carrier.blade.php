                    ...
                    <div class="col-xl-4 col-lg-6 col-md-8">
                        <div class="login-title text-center">
                            <h1><a href="{{ route('frontend.home') }}"><img class="login-logo"
                                        src="{{ asset('assets/common/img/logo/loder.png') }}" alt=""></a>Carrier Log
                                In
                            </h1>
                        </div>
                        <div class="pt-10">
                            <h5 class="text-danger text-center">Enter your US DOT Number and Verify Code *</h5>
                        </div>
                        <form class="form-login contact_form" method="post" action="{{ route('login.carrier.perform') }}">
                            @csrf
                            <div class="row d-flex justify-content-center">
                                <div class="col-8 col-md-12 mb-3">
                                    <div class="form-group">
                                        <input class="form-control" name="dot_number" value="{{ old('dot_number') }}"
                                            onfocus="this.placeholder = ''"
                                            onblur="this.placeholder = 'Enter Your US DOT Number'"
                                            placeholder="Enter Your US DOT Number" required>
                                        @if ($errors->has('dot_number'))
                                            <span class="text-danger text-left">{{ $errors->first('dot_number') }}</span>
                                            <input type="hidden" class="errorMsg"
                                                value="{{ $errors->first('dot_number') }}">
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="col-8 col-md-12">
                                    ...
                                    <p class="mb-0"><a href="{{ route('frontend.home') }}"><i
                                                class="fa fa-arrow-circle-left"></i> Back to Main
                                            Page</a></p>
                                    <p class="mb-0"><a href="{{ route('login.driver') }}"><i
                                                class="fa fa-arrow-circle-left"></i> To Driver Login Page</a></p>
                                </div>
                            </div>
                            <div class="login-link text-center mt-5">
                                <a href="{{ route('frontend.home') }}">Quick Freight Enterprise INC @
                                    {{ date('Y') }}</a>
                            </div>
                        </form>
                    </div>
